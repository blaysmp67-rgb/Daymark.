...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const configuredUrl = Deno.env.get("DAYMARK_APP_URL")?.trim();
    const requestOrigin = req.headers.get("origin")?.trim();
    const appUrl = configuredUrl || (requestOrigin?.startsWith("https://") ? requestOrigin : undefined);
    ...
    if (appUrl) payload.url = appUrl.endsWith("/") ? appUrl : `${appUrl}/`;
